...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      Connection(std::unique_ptr<socket_type> &&socket) noexcept : socket(std::move(socket)), timeout_idle(0), strand(this->socket->get_io_service()), closed(false) , fetch_snapshot_ts(0), is_sp(false) {}

     ~Connection() { malloc_trim(0); }

      std::string method, path, query_string, http_version;
      std::string recording_filename, config_kvps;
      std::vector<int> topics_subscribed;
      double fetch_snapshot_ts = 0;
      int lidar_id = 0;
      int return_count = 0;
      int play_speed = 10000;
      int pre_record_frames = 0;
      double roi_center_horz, roi_center_vert;
      int dynamic_command;
      ...
      std::size_t send_queue_size() const noexcept { return send_queue.size(); }
    ...
    private:
      ...
      Connection(std::shared_ptr<ScopeRunner> handler_runner, long timeout_idle, Args &&... args) noexcept
          : handler_runner(std::move(handler_runner)), socket(new socket_type(std::forward<Args>(args)...)), timeout_idle(timeout_idle), strand(socket->get_io_service()), closed(false) {
            write_buffer_stream = std::make_shared<SendStream>();
          }
      ...
      std::shared_ptr<SendStream> write_buffer_stream;
    ...
    public:
      ...
      void write_to_buffer(const char *buf, ssize_t input_len) {
        write_buffer_stream->write(buf, input_len);
      }
      void flush_buffer() {
        send(write_buffer_stream, nullptr, 130);
        write_buffer_stream = std::make_shared<SendStream>();
      }
    ...
    private:
      ...
      bool is_sp; // true means not real websocket
      char *sp_reply;
  ...
  protected:
    ...
    void read_handshake2(const std::shared_ptr<Connection> &connection) {
      ...
      asio::async_read_until(*connection->socket, connection->read_buffer, "\r\n\r\n", [this, connection](const error_code &ec, std::size_t /*bytes_transferred*/) {
        ...
        if(!lock)
          return;
        if(!ec) {
          // std::vector<char> target(connection->read_buffer.size());
          // buffer_copy(boost::asio::buffer(target), connection->read_buffer.data());
    ...
    void read_handshake(const std::shared_ptr<Connection> &connection) {
      ...
      connection->set_timeout(config.timeout_request);
      asio::async_read_until(*connection->socket, connection->read_buffer, "\n", [this, connection](const error_code &ec, std::size_t /*bytes_transferred*/) {
          std::cout << "read " << connection->read_buffer.size() <<std::endl;
        connection->cancel_timeout();
        auto lock = connection->handler_runner->continue_lock();
        if(!lock)
          return;
        if(!ec) {
          ...
          std::string s(target.begin(), target.end());
          if (s.find("GET ") == 0) {
            std::cout << "handshake2" << std::endl;
            read_handshake2(connection);
          } else {
            ...
            for(auto &regex_endpoint : endpoint) {
              if (regex_endpoint.second.is_sp) {
                connection->is_sp = true;
                if (s.find("stop") == 0 || s.find("get_server_info") == 0) {
                  std::cout << "stop! " << s << std::endl;
                  auto write_buffer = std::make_shared<asio::streambuf>();
                  std::ostream reply(write_buffer.get());
                  reply << regex_endpoint.second.sp_reply << "\n\n";
                  connection->set_timeout(config.timeout_request);
                  asio::async_write(*connection->socket, *write_buffer, [this, connection, write_buffer, &regex_endpoint](const error_code &ec, std::size_t /*bytes_transferred*/) {
                      ...
                      if(!lock)
                        return;
                      connection_close(connection, regex_endpoint.second, 1002, "response to stop");
                    });
                } else if (s.find("start/?") == 0) {
                  std::cout << "start! " << s << std::endl;
                  // finished handshake, ready to start streaming
                  connection->query_string = line.substr(7);
                  connection_open(connection, regex_endpoint.second, line);
          ...
          if(connection->generate_handshake(write_buffer)) {
            ...
            asio::async_write(*connection->socket, *write_buffer, [this, connection, write_buffer, &regex_endpoint](const error_code &ec, std::size_t /*bytes_transferred*/) {
              ...
              if(!lock)
                return;
              if(!ec) {
                connection_open(connection, regex_endpoint.second, connection->path);
                read_message(connection, regex_endpoint.second);
              }
              else
    ...
    void connection_open(const std::shared_ptr<Connection> &connection, Endpoint &endpoint, const std::string &path) const {
      connection->cancel_timeout();
      connection->set_timeout();
